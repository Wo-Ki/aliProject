...
class newOrEdit(FlaskForm):
    ...
    image1 = FileField(label=u'第一张:',
                       validators=[FileAllowed(['jpg', 'png', 'jpeg', 'gif'], u'文件为jpg,png,jpeg,gif格式')
                                   ])
...
class commentForm(FlaskForm):
    content = StringField(label=u"评论:", validators=[DataRequired(u"请输入评论"), Length(1, 200, message=u'标题不能超过20字')])
    submit = SubmitField(label=u"立即评论")
